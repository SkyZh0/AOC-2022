...
#PART-1
items: list[tuple[str, str, str, str, str]] = data
...
while todo:
    m, ore_b, cla_b, geo_b, ore, cla, obs, geo = todo.popleft()
    ore = min(maxo * (24-m), ore)
    cla = min(cost.obs_bot_cla * (24-m), cla)
    obs = min(cost.obs_bot_obs * (24-m), obs)
    ore_b = min(ore_b, maxo)
    cla_b = min(cla_b, cost.cla_bot_cla)
